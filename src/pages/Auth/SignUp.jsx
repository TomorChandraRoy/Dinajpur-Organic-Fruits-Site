...
const SignUp = () => {
  const { signUp, updateUserProfile, setUser, signInWithGoogle, loading } =
...
const signUpWithGoogle = async () => {
  try {
    ...
    // সরাসরি সার্ভারে পাঠিয়ে দিন, সার্ভারই চেক করে আপডেট বা ক্রিয়েট করবে
    const response = await axiosPublic.post("/google/user", userInfo);

    if (response.data) {
      swal("SUCCESS!", "Login successful with Google", "success");
      navigate("/");
    }

  } catch (err) {
    console.error("Google Sign Up Error:", err);
    swal("Error", "Could not sign in with Google", "error");
  }
};
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="tel" // মোবাইল কিবোর্ডে নাম্বার প্যাড আসার জন্য 'tel' ব্যবহার করা হয়েছে
                {...register("phone", {
                  required: "মোবাইল নম্বর প্রয়োজন",
                  pattern: {
                    // বাংলাদেশের ১১ ডিজিটের নাম্বারের জন্য Regex (০১৮..., ০১৭..., ইত্যাদি)
                    value: /^(?:\+88|88)?(01[3-9]\d{8})$/,
                    message: "সঠিক 11 ডিজিটের মোবাইল নম্বর লিখুন"
                  }
                })}
                className={`mt-1 block w-full px-3 py-2 border rounded-md shadow-sm sm:text-sm outline-none transition
                  ${errors.phone
            ...
            <button
              type="submit"
              className="w-full cursor-pointer flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:bg-gray-400"
            >
