          ...
          {categories.map((cat) => (
            <SwiperSlide key={cat.name}>
              <button
                ...
                className="w-full rounded-[18px] border border-gray-200 bg-white px-[14px] py-[20px] text-center shadow-[0_6px_18px_rgba(0,0,0,0.08)] transition-all hover:-translate-y-1 hover:border-green-600 hover:shadow-[0_12px_28px_rgba(0,0,0,0.12)]"
              >
                <img
                  src={cat.image}
                  alt={cat.name}
                  className="mx-auto mb-2 h-[48px] w-[48px] rounded-full border border-gray-200 object-cover"
                  loading="lazy"
                />
                <h3 className="text-[13px] font-semibold text-gray-900">{cat.name}</h3>
                <span className="text-[10.5px] text-gray-500">{cat.count} products</span>
              </button>
            </SwiperSlide>
          ))}
        </Swiper>
